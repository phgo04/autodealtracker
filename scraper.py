#!/usr/bin/env python3
"""
AutoTrader.ca scraper for Mazda CX-5 listings in Ontario/GTA.
Uses Selenium + Chrome (headless) to bypass Imperva bot protection.
Saves structured listing data to state/raw_listings.json.

Usage:
    python scraper.py           # Full run — saves results
    python scraper.py --test    # First page only, prints summary, no save
"""
...
from config import (
    SEARCH_BASE_URL, MAX_PAGES, YEAR_MIN, YEAR_MAX,
    RAW_LISTINGS_FILE, STATE_DIR, BUY_NOW,
)
...
def make_driver() -> webdriver.Chrome:
    ...
    # Hide the webdriver property — Incapsula checks for it
...
def _normalise_next(raw: dict, today: str) -> dict:
    # ID: prefer UUID 'id', fall back to numeric from URL
    ...
    # Price: stored as object {"priceFormatted": "$ 32,998", ...}
    ...
    if isinstance(price_obj, dict):
        ...
        raw_price = price_obj   # already a number or string
    ...
    # CPO: check specialConditions array for "certified" type
...
def extract_from_html(html: str, today: str) -> list:
    ...
    if not cards:
        cards = [
            a for a in soup.find_all("a", href=re.compile(r"/a/mazda/cx-5/"))
    ...
    for card in cards:
        ...
        if m:
            year = int(m.group(1))
            after = re.sub(rf".*{year}\s*mazda\s*cx-?5\s*", "", title, flags=re.I).strip()
...
def build_url(page_num: int) -> str:
    """Page 1 = base URL with no params; page 2+ = ?page=N"""
    if page_num <= 1:
        return SEARCH_BASE_URL
    return f"{SEARCH_BASE_URL}?page={page_num}"


# ── Core scrape loop ──────────────────────────────────────────────────────────

def scrape(test_mode: bool = False) -> list:
    today = date.today().isoformat()
    all_listings: list = []
    seen_ids: set = set()

    print(f"\nCX-5 Tracker - AutoTrader Scrape - {today}")
    print(f"Search: {SEARCH_BASE_URL}")
    ...
    try:
        # Warmup — visit homepage to establish session and pass Incapsula check
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            if added == 0:
                # If an entire page added nothing new (all out-of-year or duplicates), stop
                break
...
def print_summary(listings: list) -> None:
    if not listings:
        print("\nNo listings found.")
        return
    ...
    threshold = BUY_NOW["used_2024"]
    candidates = [
        l for l in listings
        if l.get("condition") == "used"
        and l.get("year") == 2024
        and l.get("price") and l["price"] <= threshold["max_price"]
        and l.get("km") is not None and l["km"] <= threshold["max_km"]
    ]
    print(f"\n  BUY NOW (2024 used <=${threshold['max_price']:,} / <={threshold['max_km']:,} km): "
          f"{len(candidates)} found")
    for c in candidates[:5]:
        print(f"    >> {c.get('year')} {c.get('trim')} - ${c['price']:,} / {c['km']:,} km"
...
def main():
    parser = argparse.ArgumentParser(description="AutoTrader CX-5 scraper")
    parser.add_argument(
    ...
    listings = scrape(test_mode=args.test)
    ...
    with open(RAW_LISTINGS_FILE, "w", encoding="utf-8") as f:
        json.dump(listings, f, indent=2, ensure_ascii=False)
    print(f"Saved {len(listings)} listings -> {RAW_LISTINGS_FILE}\n")
